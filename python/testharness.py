...
def LandPlane(plane,destination,destLocation):
    ...
    requests.delete("%s/planes/%s/route/destination" % (url,plane['callsign']), verify=False)
    requests.put( "%s/planes/%s/location/%f,%f/%d/%s"  % (url,plane['callsign'],
    plane['currentLocation'][0], plane['currentLocation'][1],plane['heading'],destination), verify=False)

    cargoes =  requests.get("%s/cargo/location/%s" % (url,plane['callsign']), verify=False).json()
    ...
    if  cargoes != None :
        for cargo in cargoes:
            if cargo['destination'] == destination:
                ...
                requests.put("%s/cargo/%s/location/%s" % (url,cargo['id'],destination), verify=False)
                requests.put("%s/cargo/%s/delivered" % (url,cargo['id']), verify=False)
                
            else:
                if cargo['destination'] in plane['route'] :
                    print("Keeping cargo %s onboard for delivery to %s" % (cargo['id'],cargo['destination']))
                else:
                    requests.delete("%s/cargo/%s/courier" % (url,cargo['id']), verify=False)
                    requests.put("%s/cargo/%s/location/%s" % (url,cargo['id'],destination), verify=False)
    ...
    sitecargo =  requests.get("%s/cargo/location/%s" % (url,destination), verify=False).json()

    if sitecargo != None:
        for cargo in sitecargo:
            if cargo['courier'] == plane['callsign']:
                print("On Loading %s for %s" % (cargo['id'],cargo['destination']))
                requests.put("%s/cargo/%s/location/%s" % (url,cargo['id'], plane['callsign']), verify=False)
...
def movePlane(plane,destination):
    ...
    if abs(dx) < 0.5 and abs(dy) < 0.5:
       ...
       LandPlane(plane,destination,destLocation)
        ...
        plane['currentLocation'][1],plane['heading']), verify=False)
...
def run_simulation():
    ...
    cities = requests.get(url + "/cities", verify=False).json()
    for city in cities:
        city_locations[city['name']] = city['location']

    while True:
        planes = requests.get(url + "/planes", verify=False).json()
        for plane in planes:
        ...
        if arrive_at != send_to:
            cargoes =  requests.get("%s/cargo/location/%s" % (url,arrive_at), verify=False).json()
            
            if len(cargoes) < 20 :
                    print("Parcel arrives at %s for %s, %d pending deliviery" % (arrive_at,send_to,len(cargoes)+1))
                    print(requests.post("%s/cargo/%s/to/%s" % (url,arrive_at,send_to), verify=False).json())
            time.sleep(0.5)
