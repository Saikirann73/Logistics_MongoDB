...
function getSelected()
{
  ...
  if(app.selectedType == "plane") {
    app.planes.forEach(  plane => { if (plane.callsign == app.selected){
      rval = plane
    }
    })
  } else {
    app.cities.forEach(  city => { if (city.name == app.selected) {
      rval = city    
    }});
...
{
  app.avgDeliveryTime = await getAverageDeliveryTime();
}
...
async function onLoad()
{
    app = new Vue({
      ...
      setInterval( GetPlanes, 500 ) 
      setInterval( GetAverageDeliveryTime, 10000)
}
